...
const HighRiskMonitoringDashboard = () => {
  ...
  useEffect(() => {
    const fetchMonitoringData = async () => {
      try {
        ...
        const response = await fetch(`${apiUrl}/api/monitoring/scan`);
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          ...
          <div className="text-lg text-gray-700">Scanning patients for high-risk indicators...</div>
        </div>
      </div>
    );
  }
  ...
  const alertCodeNames: Record<string, string> = {
    ...
    'NO_SPECIALIST': 'No Nephrologist',
    'HEAVY_PROTEINURIA': 'Heavy Proteinuria',
    'NEPHROTOXIC_MEDS': 'Nephrotoxic Meds',
    'MODERATE_HYPERKALEMIA': 'Moderate Hyperkalemia',
    'NO_SGLT2I': 'No SGLT2i',
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {Object.entries(results.alert_frequency).slice(0, 10).map(([code, count]) => (
              <div key={code} className="bg-gray-50 rounded-lg p-3">
                ...
                <p className="text-xs text-gray-600">{alertCodeNames[code] || code}</p>
              </div>
            ))}
          </div>
        </div>
                ...
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    ...
                    <div className="flex gap-4 text-sm text-gray-600 mb-2">
                      ...
                      <span>eGFR: {patient.egfr.toFixed(1)} mL/min</span>
                      <span className={patient.egfr_trend === 'down' ? 'text-red-600 font-medium' : ''}>
                        {patient.egfr_trend} ({patient.egfr_change > 0 ? '+' : ''}{patient.egfr_change.toFixed(1)}%)
                      </span>
                    </div>
